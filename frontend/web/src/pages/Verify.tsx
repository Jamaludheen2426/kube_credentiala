...
import { VERIFICATION_API } from '../api'
...
export default function Verify() {
  ...
  const onSubmit = async () => {
    ...
    setLoading(true)
    try {
      const r = await fetch(`${VERIFICATION_API}/verify`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) })
  ...
  return (
    <div>
      <p>Paste a credential JSON to verify whether it was issued.</p>
      <textarea style={{ width: '100%', height: 160 }} value={jsonText} onChange={e => setJsonText(e.target.value)} />
      <div style={{ marginTop: 12 }}>
        <button onClick={onSubmit} disabled={loading}>{loading ? 'Verifying...' : 'Verify credential'}</button>
      </div>
